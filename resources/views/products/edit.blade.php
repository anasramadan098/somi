            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex align-items-center">
                        <p class="mb-0">{{ __('meals.edit_meal') }}</p>
                    </div>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form action="{{ route('meals.update', $meal) }}" method="POST" enctype="multipart/form-data" id="mealForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="name_ar" class="form-control" id="name_ar" value="{{ old('name_ar', $meal->name_ar) }}" required placeholder="أدخل اسم الوجبة بالعربية">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="name_en" class="form-label">{{ __('meals.meal_name_en') }} <span class="text-danger">*</span></label>
                                    <input type="text" name="name_en" class="form-control" id="name_en" value="{{ old('name_en', $meal->name_en) }}" required placeholder="Enter meal name in English">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="price" class="form-control" id="price" value="{{ old('price', $meal->price) }}" step="0.01" min="0" required placeholder="{{ __('meals.placeholders.enter_price') }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="preparation_time" class="form-control" id="preparation_time" value="{{ old('preparation_time', $meal->preparation_time) }}" min="1" placeholder="{{ __('meals.placeholders.enter_preparation_time') }}">
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="size" class="form-select" id="size">
                                        <option value="">{{ __('meals.placeholders.select_size') }}</option>
                                            <option value="sm" {{ old('size', $meal->size) == 'sm' ? 'selected' : '' }}>{{ __('meals.sizes.sm') }}</option>
                                            <option value="sm" {{ old('size', $meal->size) == 'sm' ? 'selected' : '' }}>{{ __('meals.sizes.sm') }}</option>
                                            <option value="md" {{ old('size', $meal->size) == 'md' ? 'selected' : '' }}>{{ __('meals.sizes.md') }}</option>
                                            <option value="lg" {{ old('size', $meal->size) == 'lg' ? 'selected' : '' }}>{{ __('meals.sizes.lg') }}</option>
                                    </select>
                                </div>
                            </div>
                        </div> --}}
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <textarea name="description_ar" class="form-control" id="description_ar" rows="3" placeholder="أدخل وصف الوجبة بالعربية">{{ old('description_ar', $meal->description_ar) }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="description_en" class="form-label">{{ __('meals.description_en') }}</label>
                                    <textarea name="description_en" class="form-control" id="description_en" rows="3" placeholder="Enter meal description in English">{{ old('description_en', $meal->description_en) }}</textarea>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        <div class="mb-3">
                                            ...
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox" name="is_available" id="is_available" value="1" {{ old('is_available', $meal->is_available) ? 'checked' : '' }}>
                                    ...
                                    <div class="col-6">
                                        <div class="mb-3">
                                            ...
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $meal->is_active) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const mealIngredients = @json($meal->ingredients);
    ...
    function addIngredient(ingredientData = null) {
        ...
        if (ingredientData) {
            ...
            select.value = ingredientData.id;
            quantityInput.value = ingredientData.pivot.quantity;
            if (ingredientData.pivot.notes) {
                notesInput.value = ingredientData.pivot.notes;
            }
    ...
    if (mealIngredients && mealIngredients.length > 0) {
        mealIngredients.forEach(ingredient => addIngredient(ingredient));
    }
    ...
    document.getElementById('mealForm').addEventListener('submit', function(e) {
        ...
        if (hasInvalidIngredients) {
            e.preventDefault();
            alert('{{ __("meals.please_complete_all_ingredients") }}');
            return false;
        }
    });
});
